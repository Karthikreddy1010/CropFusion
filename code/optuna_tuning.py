...
# Candidate architectures cover compact configurations (64, 32) suitable for tabular inputs
# as well as deeper multi-layer baselines.
...
def export_experiment_config(
    ...
    config_dict = {
        ...
        "max_epochs": getattr(cfg, "OPTUNA_MAX_EPOCHS", 200),
...
def run_two_stage_optuna_tuning(
    ...
    logger.info("  Max Epochs: %d", getattr(cfg, "OPTUNA_MAX_EPOCHS", 200))
    logger.info("=" * 70)
    ...
    def objective_stage1(trial: optuna.Trial) -> float:
        ...
        model_set = train_neural_cqr(
            ...
            epochs=getattr(cfg, "OPTUNA_MAX_EPOCHS", 200), batch_size=batch_size, lr=lr, weight_decay=weight_decay,
    ...
    def objective_stage2(trial: optuna.Trial) -> float:
        ...
        model_set = train_neural_cqr(
            X_train, y_train, X_val, y_val, feature_cols,
            epochs=getattr(cfg, "OPTUNA_MAX_EPOCHS", 200), batch_size=opt_bs, lr=opt_lr, weight_decay=opt_wd,
