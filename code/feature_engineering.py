...
def build_split_aware_lags(
    ...
    ----------
    train_df : pd.DataFrame
        Training/Fit split partition.
    val_df : pd.DataFrame
        Validation/Dev split partition.
    test_df : pd.DataFrame
    ...
    cal_df : Optional[pd.DataFrame]
        Optional calibration split partition.

    Returns
    -------
    Tuple with lag features attached to each split partition.
    """
    tr = train_df.copy()
    va = val_df.copy()
    ca = cal_df.copy() if cal_df is not None else None
    te = test_df.copy()

    if target_col not in tr.columns or "GEOID" not in tr.columns or "Year" not in tr.columns:
        return (tr, va, ca, te) if ca is not None else (tr, va, te)

    if split_type == "temporal":
        # In temporal split: combine past chronology
        dfs_to_combine = [tr, va, ca, te] if ca is not None else [tr, va, te]
        combined = pd.concat(dfs_to_combine, axis=0).sort_values(["GEOID", "Year"])
        ...
        if ca is not None:
            ca["Yield_lag1"] = combined.loc[ca.index, "Yield_lag1"]
            ca["Yield_lag2"] = combined.loc[ca.index, "Yield_lag2"]
        ...
        dfs_list = [tr, va, ca, te] if ca is not None else [tr, va, te]
        for df_part in dfs_list:
    ...
    logger.info("Split-aware lag features constructed (split_type='%s', tr=%d, va=%d, cal=%s, te=%d)",
                split_type, len(tr), len(va), len(ca) if ca is not None else "None", len(te))
    return (tr, va, ca, te) if ca is not None else (tr, va, te)
...
def build_split_aware_rolling_features(
    ...
    if split_type == "temporal":
        ...
        for var in climate_vars:
            ...
            if ca is not None:
                ca[r2_col] = combined.loc[ca.index, r2_col]
                ca[r3_col] = combined.loc[ca.index, r3_col]
        ...
        # All non-held-out-state partitions (fit / dev / cal) belong to the same
        # five states, so their rolling climate windows are built together over
        # the full development-state chronology. The held-out state is handled
        # separately below and never contributes to them.
        #
        # The calibration partition used to be omitted here entirely, which left
        # `cal` without any Rolling*_mean columns and made scaling fail once a
        # calibration fold was introduced.
        dev_parts = [p for p in (tr, va, ca) if p is not None]
        tr_va = pd.concat(dev_parts, axis=0).sort_values(["GEOID", "Year"])
        for var in climate_vars:
            r2_col = f"Rolling2yr_{var}_mean"
            r3_col = f"Rolling3yr_{var}_mean"
            ...
            for part in dev_parts:
                part[r2_col] = tr_va.loc[part.index, r2_col]
                part[r3_col] = tr_va.loc[part.index, r3_col]
    ...
    logger.info("Split-aware rolling climate features constructed (split_type='%s', tr=%d, va=%d, cal=%s, te=%d)",
                split_type, len(tr), len(va), len(ca) if ca is not None else "None", len(te))
    return (tr, va, ca, te) if ca is not None else (tr, va, te)
