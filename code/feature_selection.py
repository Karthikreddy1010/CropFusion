...
def select_features(
    ...
    Notes
    -----
    ``report_suffix`` disambiguates the exported artefacts when selection is
    re-run per LOSO fold (each fold must select features from its own training
    states only, so each fold writes its own report rather than overwriting the
    main temporal one).
    """
    logger.info("=" * 60)
    ...
    exclude = (
        ...
        # Bookkeeping columns attached by the LOSO fold generator. `Fold` is an
        # integer and would otherwise be offered to the selectors as a candidate
        # predictor when selection runs per fold.
        + ["Fold", "HeldOutState", "obs_id"]
    )
    feature_cols = [
    ...
    save_report_csv(ranks_df, f"feature_selection_ranks{report_suffix}.csv")
...
"""
    ...
    save_report_markdown(md, f"feature_selection_report{report_suffix}.md")
    save_report(report, f"feature_selection_report{report_suffix}.json")
    report["n_candidates"] = len(feature_cols)
    report["n_consensus_selected"] = len(selected_features)
    logger.info("Consensus feature selection: %d candidates -> %d consensus-selected (report%s)",
                len(feature_cols), len(selected_features), report_suffix or " [main]")
