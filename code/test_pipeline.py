...
- Test 0: Authoritative 4-way temporal split protocol (FIT: 1985-2013, DEV: 2014-2015, CAL: 2016-2018, TEST: 2019-2023)
...
- Test G: Historical normal isolation (changing DEV/CAL/TEST climate cannot alter FIT normals)
- Test H: Temporal historical normal (temporal normals use only 1985–2013 fit observations)
...
from splitting import (
    temporal_split_4way, temporal_split, loso_cv_folds, random_row_split, random_grouped_county_split,
...
class TestPaper3SplitAwarePipeline(unittest.TestCase):
    ...
    # ── Test 0: Authoritative 4-Way Temporal Split Protocol ────────────────────
    def test_0_temporal_split_4way_authoritative_protocol(self):
        """Test 0: Primary temporal split protocol must partition into FIT (1985-2013),
        DEV (2014-2015), CAL (2016-2018), and TEST (2019-2023) with zero overlap."""
        fit_df, dev_df, cal_df, test_df = temporal_split_4way(self.dummy_df, target=cfg.PRIMARY_TARGET)

        self.assertEqual(fit_df["Year"].min(), 1985)
        self.assertEqual(fit_df["Year"].max(), 2013)
        self.assertEqual(dev_df["Year"].min(), 2014)
        self.assertEqual(dev_df["Year"].max(), 2015)
        self.assertEqual(cal_df["Year"].min(), 2016)
        self.assertEqual(cal_df["Year"].max(), 2018)
        self.assertEqual(test_df["Year"].min(), 2019)
        self.assertEqual(test_df["Year"].max(), 2023)

        fit_pairs = set(zip(fit_df["GEOID"], fit_df["Year"]))
        dev_pairs = set(zip(dev_df["GEOID"], dev_df["Year"]))
        cal_pairs = set(zip(cal_df["GEOID"], cal_df["Year"]))
        test_pairs = set(zip(test_df["GEOID"], test_df["Year"]))

        self.assertTrue(fit_pairs.isdisjoint(dev_pairs))
        self.assertTrue(fit_pairs.isdisjoint(cal_pairs))
        self.assertTrue(fit_pairs.isdisjoint(test_pairs))
        self.assertTrue(dev_pairs.isdisjoint(cal_pairs))
        self.assertTrue(dev_pairs.isdisjoint(test_pairs))
        self.assertTrue(cal_pairs.isdisjoint(test_pairs))
    ...
    def test_G_historical_normal_isolation(self):
        """Test G: Changing DEV/CAL/TEST climate values cannot change FIT-derived normals."""
        fit_df, dev_df, cal_df, test_df = temporal_split_4way(self.dummy_df, target=cfg.PRIMARY_TARGET)
    ...
    def test_H_temporal_historical_normal(self):
        """Test H: Temporal normals use only 1985–2013 fit observations (Zero Leakage)."""
        fit_df, dev_df, cal_df, test_df = temporal_split_4way(self.dummy_df, target=cfg.PRIMARY_TARGET)
        prep = TrainFittedPreprocessor(target_col=cfg.PRIMARY_TARGET)
        prep.fit(fit_df)

        for geoid in fit_df["GEOID"].unique():
            tr_sub = fit_df[fit_df["GEOID"] == geoid]
    ...
    def test_O2_configuration_consistency(self):
        ...
        self.assertEqual(getattr(cfg, "BASELINE_MAX_EPOCHS", None), 200)
        self.assertEqual(getattr(cfg, "OPTUNA_MAX_EPOCHS", None), 200)
        self.assertEqual(getattr(cfg, "MAX_TUNING_EPOCHS", None), 200)
        self.assertEqual(getattr(cfg, "LOSO_MAX_EPOCHS", None), 200)
        self.assertEqual(cfg.RANDOM_SEED, 42)
        # Authoritative 4-way protocol (§5.1)
        self.assertEqual(cfg.FIT_YEARS, (1985, 2013))
        self.assertEqual(cfg.DEV_YEARS, (2014, 2015))
        self.assertEqual(cfg.CAL_YEARS, (2016, 2018))
    ...
    # ── Test Legacy: Backward-Compatibility Check ─────────────────────────────
    def test_legacy_temporal_split_compatibility(self):
        """Test Legacy: Verify legacy temporal_split() remains backward-compatible but flagged."""
        tr, va, te = temporal_split(self.dummy_df, target=cfg.PRIMARY_TARGET)
        self.assertEqual(tr["Year"].min(), 1985)
        self.assertEqual(tr["Year"].max(), 2015)
        self.assertEqual(va["Year"].min(), 2016)
        self.assertEqual(va["Year"].max(), 2018)
        self.assertEqual(te["Year"].min(), 2019)
        self.assertEqual(te["Year"].max(), 2023)
