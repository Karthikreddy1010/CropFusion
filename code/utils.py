...
def file_sha256(path, chunk_size: int = 1024 * 1024) -> str:
    """Full-file SHA-256 checksum, streamed in chunks.

    The previous implementation hashed only ``f.read(1024 * 1024)`` -- the first
    1 MB -- while labelling the result a dataset checksum. On a 24 MB dataset
    that covers about 4% of the bytes, so any change after the first megabyte
    (including the entire test period) produced an identical "checksum" and the
    reproducibility claim was unfounded. This reads the whole file.
    """
...
def file_partial_md5(path, n_bytes: int = 1024 * 1024) -> str:
    """MD5 of the FIRST ``n_bytes`` only -- explicitly a partial checksum.

    Kept for cheap change-detection on large artefacts. Never present the result
    as a full-file checksum; the name says what it is.
    """
    import hashlib as _h
    hasher = _h.md5()
    with open(path, "rb") as f:
...
def generate_reproducibility_report(
    ...
    if cfg.DATA_FILE.exists():
        dataset_hash = file_sha256(cfg.DATA_FILE)
        dataset_size_bytes = cfg.DATA_FILE.stat().st_size
    ...
    report_data = {
        ...
        "settings": {
            ...
            "dataset_sha256_full_file": dataset_hash,
            "dataset_size_bytes": dataset_size_bytes,
            "dataset_checksum_scope": "complete file (all bytes)",
...
def validate_artifact_integrity() -> Dict[str, Any]:
    ...
    required_artifacts = [
        ...
        ("reports", "feature_selection_report.json"),
        ("reports", "feature_selection_ranks.csv"),
        ("reports", "feature_selection_report.md"),
        ("reports", "model_decisions_lock.json"),
        ("reports", "model_decisions_lock.md"),
    ...
    optional_artifacts = [
        ("reports", "feature_selection_stability.json", "Optional exploratory stability sub-study; consensus feature selection reported in feature_selection_report.json"),
        ("reports", "feature_selection_stability.csv", "Optional exploratory stability sub-study; consensus ranks reported in feature_selection_ranks.csv"),
        ("reports", "feature_selection_stability.md", "Optional exploratory stability sub-study; consensus analysis reported in feature_selection_report.md"),
        ("reports", "xgboost_quantile_report.json", "Optional standalone XGBoost quantile report; multi-backbone benchmark evaluated in evaluation_report.json"),
    ]
    ...
    for subfolder, filename in required_artifacts:
        ...
        if exists:
            ...
            sha256_hash = file_sha256(target_path)
        else:
            missing_count += 1
        ...
        audits.append({
            ...
            "SHA256_Full_File": sha256_hash,
            "Creation_Timestamp": timestamp,
            "Status": "PASSED" if exists and size_bytes > 0 else "MISSING",
        })

    for subfolder, filename, reason in optional_artifacts:
        target_path = cfg.OUTPUT_DIR / subfolder / filename
        exists = target_path.exists()
        ...
        audits.append({
            "Artifact": filename,
            "Category": subfolder,
            "Requirement": "Optional/Exploratory",
            "Exists": exists,
            "Size_Bytes": size_bytes,
            "SHA256_Full_File": sha256_hash,
            "Creation_Timestamp": timestamp,
            "Status": "PASSED (OPTIONAL)" if exists and size_bytes > 0 else "OPTIONAL_NOT_APPLICABLE",
            "Notes": reason,
        })
...
- **Core Methodology Artifact Completion Rate**: `{completion_pct}%` ({len(required_artifacts) - missing_count} / {len(required_artifacts)} Present)
- **Total Missing Required Artifacts**: `{missing_count}`
...
"""
    for a in audits:
        md += f"| {a['Category']} | {a['Artifact']} | {a.get('Requirement', 'Required')} | {a['Exists']} | {a['Size_Bytes']} | `{a['SHA256_Full_File'][:12]}` | {a['Status']} |\n"
