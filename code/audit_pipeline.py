...
def run_baseline_reproduction_audit(
    ...
    baseline_max_epochs = getattr(cfg, "BASELINE_MAX_EPOCHS", 200)
...
These invariants are **not** verified by this script. The authoritative checks
live in the main pipeline and write their own evidence:

| Invariant | Where it is actually verified |
| :--- | :--- |
| No train/dev/cal/test observation overlap | `reports/leakage_provenance_audit.json` (row-identity check on `obs_id`) |
| Train-only preprocessing and scaling | `methodology_validation_report.json` checks M7, M8 |
| Reproducible split assignments | `methodology_validation_report.json` check M6 |
| Deterministic seeding | `reports/reproducibility_report.json` |
| Historical comparison | the delta table above |

Previously this section printed five hard-coded **PASS** lines, including
"Zero data leakage", regardless of what any check found. Read the artefacts
named above instead.
"""
