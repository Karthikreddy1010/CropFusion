...
| M12 | Ablation configurations 3, 4 and 5 are mutually distinct | **PASS** | 2_plus_cdhw vs 3_plus_cqr: PASS (interval head added (post-hoc: the point path is unchanged by design)); 3_plus_cqr vs 4_plus_aci: PASS (static conformal -> ACI (calibration only)); 4_plus_aci vs 5_full_joint: PASS (post-hoc -> joint end-to-end training) | `ablation_report.json` |
...
| M14 | No artefact claims seven LOSO folds (the protocol has six) | **PASS** | no stale seven-fold reference found in code or reports; config.LOSO_STATES has 6 states | `code/*.py + outputs/reports/*` |
| M15 | Significance testing accounts for temporal/spatial dependence | **PASS** | O5 primary inference = OLS with cluster-robust (CR1) standard errors clustered by year (5 clusters, effective n = 35 from 2345 rows); O6 Friedman executed = True, block unit = year, k = 6 with q_alpha = 2.85 | `objective_o5_report.json + objective_O6_report.json` |
